...
export class AcmeHandler extends BasePlatformHandler {
  ...
  async apply(page: Page, profile: UserProfile): Promise<ApplicationResult> {
    ...
    try {
      ...
      await this.screenshots.onError(page, error as Error, 'application_failure');
  ...
  private async handlePersonalInfoStep(page: Page, profile: UserProfile): Promise<void> {
  ...
  private async handleEducationStep(page: Page, profile: UserProfile): Promise<void> {
    ...
    try {
      ...
      await FormHelpers.selectTypeahead(
        page,
        'input[name="school"]',
        profile.education.school.substring(0, 10), // Search with partial name
      ...
      await FormHelpers.fillInput(page, 'input[name="field"]', profile.education.major);
  ...
  private async handleExperienceAndSkillsStep(page: Page, profile: UserProfile): Promise<void> {
    ...
    try {
      ...
      await experienceInput.evaluate((el, value) => {
        ...
        (el as HTMLInputElement).style.visibility = 'visible';
      ...
      await FormHelpers.selectRadio(page, 'input[name="workAuth"]', profile.authorizedToWorkInUS ? 'yes' : 'no');
      
      // Handle conditional visa sponsorship field
      if (profile.authorizedToWorkInUS) {
        // Visa field appears when work auth is yes
        await FormHelpers.selectRadio(page, 'input[name="visa"]', profile.requiresVisaSponsorship ? 'yes' : 'no');
      }
      ...
      for (const skill of availableSkills) {
        if (profile.skills.some(s => s.toLowerCase().includes(skill))) {
          ...
          await FormHelpers.selectCheckbox(page, skillSelector);
        }
      }
  ...
  private async handleResumeUploadStep(page: Page, profile: UserProfile): Promise<void> {
  ...
  private async handleReviewStep(page: Page, profile: UserProfile): Promise<void> {
    this.addStep('Review');
    ...
    try {
      ...
      await HumanBehavior.simulateReadingTime(1000); // Simulate reading review
  ...
  private async handleSubmitStep(page: Page): Promise<void> {
    this.addStep('Submit');
    ...
    try {
      ...
      await submitButton.evaluate(el => (el as HTMLElement).style.display = 'block');
      await submitButton.evaluate(el => (el as HTMLElement).style.visibility = 'visible');
      
      await submitButton.waitFor({ state: 'visible', timeout: 10000 });

      await RetryOperation.submit(async () => {
        await HumanBehavior.hoverThenClick(page, '#next-btn:has-text("Submit"), button:has-text("Submit")');
      }, 'ACME Application');
  ...
  private async clickNextButton(page: Page): Promise<void> {
    await RetryOperation.click(async () => {
      await HumanBehavior.hoverThenClick(page, '#next-btn');
    }, 'Next Button');
