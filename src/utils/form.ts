...
export class FormHelpers {
   ...
   */
  static async fillInput(page: Page, selector: string, value: string, options: {
    clear?: boolean;
    validate?: boolean;
  } = {}): Promise<boolean> {
    try {
      ...
      await HumanBehavior.humanTyping(page, selector, value);
   ...
   */
  static async selectCheckbox(page: Page, selector: string, checked: boolean = true): Promise<boolean> {
    try {
      const element = page.locator(selector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      
      await HumanBehavior.humanCheckCheckbox(page, selector);
   ...
   * Select radio button
   */
  static async selectRadio(page: Page, selector: string, value: string): Promise<boolean> {
    try {
      const radioSelector = `${selector}[value="${value}"]`;
      const element = page.locator(radioSelector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      
      await HumanBehavior.smoothScrollIntoView(page, radioSelector);
      await HumanBehavior.hoverThenClick(page, radioSelector);
   ...
   */
  static async selectDropdown(page: Page, selector: string, value: string): Promise<boolean> {
    try {
      const element = page.locator(selector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      
      await HumanBehavior.humanSelectDropdown(page, selector, value);
   ...
   */
  static async uploadFile(page: Page, selector: string, filePath: string): Promise<boolean> {
    try {
      const element = page.locator(selector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      
      await HumanBehavior.smoothScrollIntoView(page, selector);
   ...
   * Set slider value
   */
  static async setSliderValue(page: Page, selector: string, value: number): Promise<boolean> {
    try {
      const element = page.locator(selector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      ...
      await HumanBehavior.smoothScrollIntoView(page, selector);
      await HumanBehavior.moveToElement(page, selector);
   ...
   */
  static async selectTypeahead(page: Page, inputSelector: string, searchValue: string, exactMatch: string): Promise<boolean> {
    try {
      // Type search query
      ...
      if (await resultsContainer.isVisible()) {
        ...
        for (let i = 0; i < optionCount; i++) {
          ...
          if (text?.trim().toLowerCase() === exactMatch.toLowerCase()) {
            await HumanBehavior.hoverThenClick(page, optionSelector);
            return true;
          }
        }
      ...
      if (await page.locator(exactMatchSelector).isVisible()) {
        await HumanBehavior.hoverThenClick(page, exactMatchSelector);
        return true;
      }
   ...
   */
  static async selectChips(page: Page, containerSelector: string, values: string[]): Promise<boolean> {
    try {
      ...
      for (const value of values) {
        ...
        if (await chip.isVisible()) {
          await HumanBehavior.hoverThenClick(page, chipSelector);
   ...
   */
  static async setToggle(page: Page, selector: string, enabled: boolean): Promise<boolean> {
    try {
      const element = page.locator(selector);
      await element.waitFor({ state: 'visible', timeout: 10000 });
      ...
      const currentState = await element.getAttribute('aria-checked') === 'true' || 
                          ...
                          await element.evaluate((el: HTMLElement) => el.classList.contains('active') || el.classList.contains('on'));
      
      if (currentState !== enabled) {
        await HumanBehavior.hoverThenClick(page, selector);
